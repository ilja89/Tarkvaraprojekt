...
func _on_Button_pressed(choice):
	...
	if choice == "try":
		if chance <= 50 + CharacterStats.Strength * 2 + CharacterStats.Agility * 2 + CharacterStats.Luck:
			get_tree().change_scene("res://assets/Scenes/Main scenes/3/39/Scene.tscn")
		else:
			get_tree().change_scene("res://assets/Scenes/Endings/0053/Scene.tscn")
	if choice == "run":
		if chance <= 60 + CharacterStats.Agility * 3 + CharacterStats.Luck:
